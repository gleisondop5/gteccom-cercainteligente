...
class Agent:
    ...
    def __init__(self, tag_slug):
        ...
        try:
            makedirs(self._local_storage_dir) #cria a pasta para cada câmera em ~/agent-storage/local 
        except OSError as error:
            if error.errno != errno.EEXIST: raise

        self._final_storage_dir = path.join(path.expanduser('~'), 'agent-storage', 'final')
        try:
            makedirs(self._final_storage_dir) #cria a pasta ~/agent-storage/final
        except OSError as error:
            if error.errno != errno.EEXIST: raise
    ...
    def run(self, monitor, video_path, database_host, database_port, database_name, database_user, database_password, num_frame_workers=2, num_local_storage_workers=2, frame_queue_size=120, storage_queue_size=120):
        ...
        self._keep_running = True #está rodando
        try:
    ...
    def _run_communication(self, ws_uri_mask): #cria o evento comunication
        self.logger.debug('%s\tcommunication thread - begin' % self.tag_slug)
        try:
            self._communication_event_loop = asyncio.new_event_loop() # cria um novo objeto de loop de evento.
            try:
                asyncio.set_event_loop(self._communication_event_loop) # define o loop como um loop de evento atual para o thread atual do sistema operacional.
                self._communication_event_loop.run_until_complete(self._communication_loop(ws_uri_mask)) #Executa até que o futuro (resultado eventual de uma operação assíncrona) seja concluído.
            finally:
    ...
    #@asyncio.coroutine
    async def _communication_loop(self, ws_uri_mask):
        ...
        try:
            ...
            while self._keep_running: #enquanto estiver rodando ...
                try:
                    self.logger.info('%s\twebsocket - connecting...' % self.tag_slug)
                    #self._ws = websockets.connect(ws_uri_mask % ((self.status() & Agent.STATUS_CAMERA_RUNNING) == Agent.STATUS_CAMERA_RUNNING)) #abre a conexão com um WebSocket
                    ...
                    try:
                        ...
                        while self._keep_running and not self._ws.closed:
                            ...
                            if callback is not None and data.get('target', self.tag_slug) == self.tag_slug: callback(self, data)
